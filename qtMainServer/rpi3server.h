...
*/
    ...
    bool doStartServer(QHostAddress addr = QHostAddress::Any, qint16 port = 8080, QString DevicePort = ""); // Запуск сервера
    bool doStopServer();    // Остановка сервера
    void SendMessageToClient(quint8 command, const QString& str);  // Отправка сообщения клиенту

signals:
    void AddLogMessage(QString text, QColor color = Qt::black);
