...
bool CRpi3Server::doStartServer(QHostAddress addr, qint16 port, QString DevicePort)
{
    // Стартуем сервер
    if (!listen(addr, port))
    {
        AddLogMessage("Server NOT started at" + addr.toString() + ":" + QString::number(port));
        return false;
    }
    AddLogMessage( "Server started at" + addr.toString() + ":" + QString::number(port));

    // TODO: Здесь так же подключаемся к устройству
    // Стартуем устройство
    if( DevicePort != "" )
    {
        _cfgDevice._name = DevicePort;
        _device.setPortName(DevicePort);
    }
    ...
    if ( !_device.open(QSerialPort::ReadWrite) )
    {
        ...
        AddLogMessage("Device" + _device.portName() + " open fail! ");
    }
    else
    {
        qDebug() << "Device on " << _device.portName() << " connected!";
        AddLogMessage("Device" + _device.portName() + " connected! ");
        //_device.clear();
    }
...
//    qDebug() << "Received command " << command;
    ...
    switch(command)
    {
        case cmdAutchReq:
        {
            ...
            AddLogMessage("Auth - OK!");
            // отправляем клиенту его имя
            SendMessageToClient(cmdAutchReq, name + " - OK!");
            // а так же передаем текущие параметры
            // TODO: отправляем клиенту параметры устройства
            QByteArray block;
            QDataStream out(&block, QIODevice::WriteOnly);
            out.setVersion(QDataStream::Qt_4_8);
            //резервируем место под размер блока
        ...
        {        
            // тут принимаем сообщение от клиента
            QString msg;
            in >> msg;
            AddLogMessage("Get message from client:"+ msg);
        ...
        case cmdFileFromClient:
        {
            ...
            AddLogMessage("Get file to device:" + msg);
            // тут принимаем файл
        ...
        case cmdGetDeviceParam:
        {
            ...
            QString port;
            in >> port;
            AddLogMessage("Get settings from device:" + port);
            ...
            if ( port != "" )
            {
                ...
                SendMessageToClient(cmdMsgInfoToClient, "Not this command in reliase...wait in next build");

            }
            // TODO: отправляем клиенту параметры текущего порта
            QByteArray block;
            QDataStream out(&block, QIODevice::WriteOnly);
            out.setVersion(QDataStream::Qt_4_8);
            //резервируем место под размер блока
        ...
        case cmdSetDeviceParam:
        {
            ...
            in >> _settings;
            AddLogMessage("Get settings to device:" + _settings._name);
        ...
        case cmdGetDeviceList:
        {
            ...
            AddLogMessage("Get devices list:");
        ...
        {
            // тут принимаем данные для устройства
            QString msg;
            in >> msg;
            AddLogMessage("Get message to Device:"+ msg);
            qDebug() << "Get cmdMsgFromClient:" << msg;
            QByteArray d;
            d.append(msg);
            ...
            else
            {
                ...
                QString msg;
                msg = "Error send command to device: "+_device.portName() + " is not open!";
                qDebug() << msg;
    ...
    default:
        //
        AddLogMessage("Unknow command:"+QString::number( command ));

        qDebug() << "Unknow command: " + QString::number( command );
        QString msg;
        in >> msg;
        AddLogMessage("Message:"+msg);
        qDebug() << "Message: " + msg;
    }
...
void CRpi3Server::GetDataFromDevice()
{
    ...
    AddLogMessage("Get data from Serial:" + msg);
...
bool CRpi3Server::SetCfgDevice(CSettingsDevice &cfg)
{
    ...
    AddLogMessage("Set new settings to device:" + cfg._name);
...
void CRpi3Server::onDeviceError(QSerialPort::SerialPortError error)
{
    ...
    if( _sok )
    {
        AddLogMessage("Error on device:" + QString::number(error) + ":" + _device.errorString(), Qt::red);
